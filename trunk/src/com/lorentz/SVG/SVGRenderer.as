package com.lorentz.SVG {
	...
	public class SVGRenderer extends Sprite {
		...
		protected var _pixelHint:Boolean = false;
		protected var _imageCache:Object;
		
		public function SVGRenderer(svg:Object, renderNow:Boolean = true, pixelHint:Boolean = false, imageCache:Object = null){
			
			_pixelHint = pixelHint;
		...
		private function visit(elt:Object):Sprite {
			...
			if(obj!=null){
				...
				if(elt.scale9Grid!=null && !numChildren){
					var props:Array = StringUtil.trim(String(elt.scale9Grid)," ").split(",");
					var scale9:Rectangle = new Rectangle(props[0] || 0, props[1] || 0, props[2] || 0, props[3] || 0);
					var bounds:Rectangle = obj.getRect(obj);
					bounds.inflate(-1, -1);
					scale9 = scale9.intersection(bounds);
					if (scale9.width && scale9.height) {
						//obj.graphics.lineStyle(0, 0xFFFF00);
						//obj.graphics.drawRect(scale9.x, scale9.y, scale9.width, scale9.height);
						obj.scale9Grid = scale9;
					}
				}
		...
		private function visitImage(elt:Object):Sprite {
			...
			if (_imageCache && _imageCache[elt.href])
				loader.loadBytes(_imageCache[elt.href]);			
			else if(elt.href.match(/^data:[a-z\/]*;base64,/))
		...
		private function applyFilters(s:Sprite, elt:Object):void {
			elt;
		}
		...
		private function lineStyle(s:Sprite, elt:Object):void {
			...
			if(!noStroke && elt.finalStyle.stroke.indexOf("url")>-1){
				...
				if(grad!=null){
					switch(grad.type){
						...
						case GradientType.RADIAL: {
							...
							if(grad.r==0)
								s.graphics.lineStyle(w, grad.colors[grad.colors.length-1], grad.alphas[grad.alphas.length-1], _pixelHint, "normal", stroke_linecap, stroke_linejoin);
							else
				...
				s.graphics.lineStyle(w, color, stroke_opacity, _pixelHint, "normal", stroke_linecap, stroke_linejoin);
		}
