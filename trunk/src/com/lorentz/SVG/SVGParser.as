package com.lorentz.SVG {
	...
	public class SVGParser {
		...
		private function visit(elt:XML):Object {
			...
			if ("@scale-9-grid" in elt)
				obj.scale9Grid = String(elt["@scale-9-grid"]);
		...
		private function parseFilter(id:String):Array{
			id = StringUtil.ltrim(id, "#");
			
			if(svg_object.filters[id]!=null)
				return svg_object.filters[id];
				
			var filter:Array;
			
			var xml_filter:XML = svg..*.(attribute("id")==id)[0];
		
			//inherits the href reference
			var xlink:Namespace = new Namespace("http://www.w3.org/1999/xlink");
			if(xml_filter.@xlink::href.length()>0){
				filter = parseFilter(xml_filter.@xlink::href);
			}
			//
			...
			//Save the filterient definition
			svg_object.filters[id] = filter;
			//
		
		
			// iterate over the tags in the xml_filter, come up with a list
			var xml_filter_item:XML, filter_item:BitmapFilter;
			for each (xml_filter_item in xml_filter) {
				switch (xml_filter_item.localName()) {
					case "feBlend":
					break;
					case "feColorMatrix":
					break;
